...
class ObraController:
    def get_obras(db: Session, params: Params):
        obras = db.query(ObrasModel).filter(ObrasModel.deleted_at == None)
        return paginate(obras, params)
    ...
    def get_obra_by_name(nombre_obra: str, db: Session):
        ...
        nombre_pattern = f"%{nombre_obra}%" if nombre_obra else "%"
        
        obra = db.query(ObrasModel).filter(ObrasModel.nombre_obra.ilike(nombre_pattern)).one_or_none()
        if obra is None:
            raise HTTPException(status_code=404, detail="Obra no encontrada")
        elif obra.deleted_at is not None:
            raise HTTPException(status_code=404, detail="Obra eliminada de forma lógica")
    ...
    def get_obras_by_autor(nombre: str, apellido: str, db: Session, params: Params):
        # buscar el id del autor
        author = AuthorController.get_author_by_name_and_lastname(nombre, apellido, db)
        
        obra = db.query(ObrasModel).filter(ObrasModel.autor_id == author.id, ObrasModel.deleted_at == None)
        if obra is None:
            raise HTTPException(status_code=404, detail="Obras no encontradas")
        return paginate(obra, params)
